...
/** 应用日志：写入程序目录 logs/；ERROR 额外写入 error 专用文件并立即刷盘 */
class AppLogger {
public:
    ...
    void debug(const QString &message) { log(QStringLiteral("debug"), message); }

    QString logFilePath() const { return m_logFilePath; }
    QString errorLogFilePath() const { return m_errorLogFilePath; }

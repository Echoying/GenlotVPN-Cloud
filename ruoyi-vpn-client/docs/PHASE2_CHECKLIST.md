...
| [x ] | S-10 | 最低客户端版本拦截         | 管理端开启后低/空版本无法云端登录，弹窗「需要升级客户端」含最低版本与下载链接；关闭后可登录；控制器/SDK 登录不被拦                     | 人验 Win/Mac                                     |
...
| [x] | W-09 | 应用图标与版本号         | exe 属性、关于页或标题显示版本                               | 版本与 git tag / 构建号一致         |
...
| 状态  | ID   | 任务          | 范围                                   | 验收标准                              |
| --- | ---- | ----------- | ------------------------------------ | --------------------------------- |
| [x] | C-01 | 网关列表轮询      | 连接控制器后轮询网关列表                         | 连接控制器后网关 IP 自动刷新至就绪               |
| [x] | C-02 | 网关切换体验      | `AppListPage` 切换网关反馈                 | 切换中有 loading，失败有 toast + error 日志 |
| [x] | C-03 | 改密流程回归      | `LoginPage` 改密弹窗                     | 改密成功/失败与 Web 行为一致                 |
...
| [x] | C-06 | 自动重连（可选）    | TCP 断线后有限次重试                         | 网络闪断可恢复，不无限循环                     |
| [x] | C-07 | 问题反馈          | 设置页提交                                   | 未登录必填账号；已登录不填账号；最多 3 张截图先传后交；超管管理端能看到并改状态。人勾（2026-08-18）。 |

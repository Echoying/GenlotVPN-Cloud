package com.ruoyi.yianlian.service.sync.handler;

import com.ruoyi.yianlian.domain.YalUserAuthBatchDTO;
import com.ruoyi.yianlian.service.IYalUserAuthService;
import com.ruoyi.yianlian.service.sync.orchestrator.SyncCommand;
import com.ruoyi.yianlian.service.sync.orchestrator.SyncConstants;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

/**
 * 用户授权同步处理器：批量保存（先删后插）+ 远程授权，远程失败回滚本地。
 */
@Component
public class YalUserAuthSyncHandler implements SyncHandler
{
    @Autowired
    private IYalUserAuthService yalUserAuthService;

    @Override
    public String bizType()
    {
        return SyncConstants.BIZ_YAL_USER_AUTH;
    }

    @Override
    public void execute(SyncCommand command) throws Exception
    {
        YalUserAuthBatchDTO dto = command.parsePayload(YalUserAuthBatchDTO.class);
        yalUserAuthService.batchSaveUserAuth(dto.getUserId(), dto.getLineId(), dto.getAuthList());
    }
}

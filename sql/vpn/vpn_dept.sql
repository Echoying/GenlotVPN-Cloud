...
-- VPN角色和部门关联表
-- ----------------------------
drop table if exists vpn_role_dept;
create table vpn_role_dept (
  role_id   bigint(20) not null comment '角色ID',
  dept_id   bigint(20) not null comment '部门ID',
  primary key(role_id, dept_id)
) engine=innodb comment = 'VPN角色和部门关联表';
...
) engine=innodb comment = 'VPN用户和角色关联表';
...
create table vpn_user (
  ...
  remark            varchar(500)    default null               comment '备注',
  primary key (user_id)
) engine=innodb auto_increment=100 comment = 'VPN用户信息表';
